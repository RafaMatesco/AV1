        ...
        if (!funcionarioLogado) {
            ...
            switch (resp) {
                case "1": {
                    ...
                    try {
                        const func = Funcionario_1.default.carregarUsuario(usuario);
                ...
                case "2": {
                    let funcsCarregados = [];
                    try {
                        funcsCarregados = Funcionario_1.default.carregarTodos();
                    }
                    catch (e) { }
                    if (funcsCarregados.length > 0) {
                        ...
                        try {
                            ...
                            const usuario = readlineSync.question("Usuario: ");
                            const senha = readlineSync.question("Senha: ");
                            const func = new Funcionario_1.default(id, nome, telefone, endereco, usuario, senha, enums_1.NivelPermissao.ADMINISTRADOR);
                            func.salvar();
    ...
    switch (opcao) {
        ...
        case "1": {
            try {
                ...
                Aeronave_1.default.verificarExistencia(codigo);
                ...
                const tipoRefinado = escolhaTipo === 0 ? enums_1.TipoAeronave.COMERCIAL : enums_1.TipoAeronave.MILITAR;
                const capacidade = Number(readlineSync.question("capacidade: "));
                const alcance = Number(readlineSync.question("alcance: "));
                const aeronave = new Aeronave_1.default(codigo, modelo, tipoRefinado, capacidade, alcance);
                aeronave.salvar();
        ...
        case "2": {
            ...
            try {
                const aeronave = Aeronave_1.default.carregar(codigoBusca);
        ...
        case "3": {
            try {
                let aeronavesCarregadas = Aeronave_1.default.carregarTodas();
        ...
        case "4": {
            ...
            try {
                const aeronave = Aeronave_1.default.carregar(codigoAtualizar);
                const modeloNovo = readlineSync.question(`Modelo (${aeronave.modelo}): `);
                if (modeloNovo.trim().length > 0)
                    aeronave.modelo = modeloNovo;
                const tipos = ["COMERCIAL", "MILITAR"];
                const escolhaTipo = readlineSync.keyInSelect(tipos, `Tipo atual: ${aeronave.tipo}. Selecionar novo tipo:`);
                if (escolhaTipo !== -1) {
                    aeronave.tipo = escolhaTipo === 0 ? enums_1.TipoAeronave.COMERCIAL : enums_1.TipoAeronave.MILITAR;
                }
                const capacidadeNova = readlineSync.question(`Capacidade (${aeronave.capacidade}): `);
                if (capacidadeNova.trim().length > 0)
                    aeronave.capacidade = Number(capacidadeNova);
                const alcanceNovo = readlineSync.question(`Alcance (${aeronave.alcance}): `);
                if (alcanceNovo.trim().length > 0)
                    aeronave.alcance = Number(alcanceNovo);
                aeronave.salvar();
        ...
        case "5": {
            ...
            const removido = Aeronave_1.default.remover(codigoRemover);
            if (removido) {
    ...
    switch (opcao) {
        ...
        case "1": {
            try {
                ...
                if (escolhaTipo === -1)
                    break;
                const tipoRefinado = escolhaTipo === 0 ? enums_1.TipoPeca.NACIONAL : enums_1.TipoPeca.IMPORTADA;
                ...
                if (escolhaStatus === -1)
                    break;
                let statusRefinado = enums_1.StatusPeca.EM_PRODUCAO;
                if (escolhaStatus === 0)
                    statusRefinado = enums_1.StatusPeca.EM_PRODUCAO;
                if (escolhaStatus === 1)
                    statusRefinado = enums_1.StatusPeca.EM_TRANSPORTE;
                if (escolhaStatus === 2)
                    statusRefinado = enums_1.StatusPeca.PRONTA;
                const peca = new Peca_1.default(nome, tipoRefinado, prazo, fornecedor, statusRefinado);
        ...
        case "2": {
            ...
            try {
                const pecas = Aeronave_1.default.carregarTodas().flatMap((a) => a.pecas);
                const peca = pecas.find((p) => p.nome === nomeBusca);
                if (peca) {
        ...
        case "3": {
            try {
                ...
                let pecasCarregadas = Aeronave_1.default.carregarTodas().flatMap((a) => a.pecas);
        ...
        case "4": {
            ...
            try {
                ...
                const peca = aeronave.pecas.find((p) => p.nome === nomeAtualizar);
                if (!peca)
                    throw new Error("Aeronave nao possui tal peca.");
                const statusOpcoes = ["EM_PRODUCAO", "EM_TRANSPORTE", "PRONTA"];
                const escolhaStatus = readlineSync.keyInSelect(statusOpcoes, `Status atual: ${peca.status}. Selecionar novo status:`);
                if (escolhaStatus !== -1) {
                    if (escolhaStatus === 0)
                        peca.atualizarStatus(enums_1.StatusPeca.EM_PRODUCAO);
                    if (escolhaStatus === 1)
                        peca.atualizarStatus(enums_1.StatusPeca.EM_TRANSPORTE);
                    if (escolhaStatus === 2)
    ...
    switch (opcao) {
        ...
        case "1": {
            try {
                ...
                if (escolhaStatus === -1)
                    break;
                let statusRefinado = enums_1.StatusEtapa.PENDENTE;
                if (escolhaStatus === 0)
                    statusRefinado = enums_1.StatusEtapa.PENDENTE;
                if (escolhaStatus === 1)
                    statusRefinado = enums_1.StatusEtapa.ANDAMENTO;
                if (escolhaStatus === 2) {
                    if (aeronave.etapas.length > 0 && aeronave.etapas[aeronave.etapas.length - 1].status !== enums_1.StatusEtapa.CONCLUIDA) {
                        console.log("Erro: Não é possível registrar esta etapa como CONCLUIDA pois a etapa anterior não está concluída.");
                        break;
                    }
                    statusRefinado = enums_1.StatusEtapa.CONCLUIDA;
                }
                const prazo = readlineSync.question("Prazo: ");
                const etapa = new Etapa_1.default(nome, statusRefinado, prazo);
        ...
        case "2": {
            ...
            try {
                const etapas = Aeronave_1.default.carregarTodas().flatMap((a) => a.etapas);
                const etapa = etapas.find((e) => e.nome === nomeBusca);
                if (etapa) {
        ...
        case "3": {
            try {
                let etapasCarregadas = Aeronave_1.default.carregarTodas().flatMap((a) => a.etapas);
        ...
        case "4": {
            ...
            try {
                ...
                const etapaIndex = aeronave.etapas.findIndex((e) => e.nome === nomeAtualizar);
                if (etapaIndex === -1)
                    throw new Error("Aeronave nao possui tal etapa.");
                ...
                if (escolhaStatus !== -1) {
                    if (escolhaStatus === 0)
                        etapa.iniciar();
                    if (escolhaStatus === 1) {
                        if (etapaIndex > 0 && aeronave.etapas[etapaIndex - 1].status !== enums_1.StatusEtapa.CONCLUIDA) {
        ...
        case "5": {
            ...
            try {
                ...
                const funcionario = Funcionario_1.default.carregar(idFuncionario);
...
function gerenciar_funcionarios() {
    console.log("=-=-=-=-=-=-=-=-=-=-=-=-=");
    ...
    switch (opcao) {
        ...
        case "1": {
            try {
                ...
                if (escolhaNivel === -1)
                    break;
                let nivelRefinado = enums_1.NivelPermissao.OPERADOR;
                if (escolhaNivel === 0)
                    nivelRefinado = enums_1.NivelPermissao.ADMINISTRADOR;
                if (escolhaNivel === 1)
                    nivelRefinado = enums_1.NivelPermissao.ENGENHEIRO;
                if (escolhaNivel === 2)
                    nivelRefinado = enums_1.NivelPermissao.OPERADOR;
                const func = new Funcionario_1.default(id, nome, telefone, endereco, usuario, senha, nivelRefinado);
                func.salvar();
        ...
        case "2": {
            ...
            try {
                const func = Funcionario_1.default.carregar(idBusca);
        ...
        case "3": {
            try {
                let funcsCarregados = Funcionario_1.default.carregarTodos();
                if (funcsCarregados.length > 0) {
        ...
        case "4": {
            ...
            try {
                const func = Funcionario_1.default.carregar(idAtualizar);
                ...
                if (telefoneNovo.trim().length > 0)
                    func.telefone = telefoneNovo;
                const niveis = ["ADMINISTRADOR", "ENGENHEIRO", "OPERADOR"];
                const escolhaNivel = readlineSync.keyInSelect(niveis, `Nivel atual: ${func.nivelPermissao}. Selecionar novo nivel:`);
                if (escolhaNivel !== -1) {
                    if (escolhaNivel === 0)
                        func.nivelPermissao = enums_1.NivelPermissao.ADMINISTRADOR;
                    if (escolhaNivel === 1)
                        func.nivelPermissao = enums_1.NivelPermissao.ENGENHEIRO;
                    if (escolhaNivel === 2)
                        func.nivelPermissao = enums_1.NivelPermissao.OPERADOR;
                }
                func.salvar();
    ...
    switch (opcao) {
        ...
        case "1": {
            try {
                ...
                if (escolhaTipo === -1)
                    break;
                let tipoRefinado = enums_1.TipoTeste.ELETRICO;
                if (escolhaTipo === 0)
                    tipoRefinado = enums_1.TipoTeste.ELETRICO;
                if (escolhaTipo === 1)
                    tipoRefinado = enums_1.TipoTeste.HIDRAULICO;
                if (escolhaTipo === 2)
                    tipoRefinado = enums_1.TipoTeste.AERODINAMICO;
                ...
                if (escolhaResultado === -1)
                    break;
                let resultadoRefinado = enums_1.ResultadoTeste.APROVADO;
                if (escolhaResultado === 0)
                    resultadoRefinado = enums_1.ResultadoTeste.APROVADO;
                if (escolhaResultado === 1)
                    resultadoRefinado = enums_1.ResultadoTeste.REPROVADO;
                const teste = new Teste_1.default(tipoRefinado, resultadoRefinado);
        ...
        case "2": {
            try {
                let testesCarregados = Aeronave_1.default.carregarTodas().flatMap((a) => a.testes);
